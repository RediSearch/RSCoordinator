...
class AggregateTestCase(BaseSearchTestCase):
    ...
    def ingest(self):
        ...
        try:
            ...
            return
        fp = bz2.BZ2File(
            '../src/dep/RediSearch/src/pytest/games.json.bz2', 'r')
        ...
        for line in fp:
            ...
            # print cmd
            self.cmd(*cmd)
        print "Finished sample data ingestion"

    def setUp(self):
        BaseSearchTestCase.setUp(self)
        self.ingest()
    ...
    def _testNoGroup(self):
        ...
        self.assertListEqual([['brand', 'Xbox', 'price', '9'], ['brand', 'Turtle Beach', 'price', '9'], [
    ...
    def _testLoad(self):
        ...
        for row in res[1:]:
            row = to_dict(row)
            self.assertIsInstance(row['brand'], basestring)
            self.assertIsInstance(row['price'], basestring)
            self.assertIsNone(row['nonexist'])
